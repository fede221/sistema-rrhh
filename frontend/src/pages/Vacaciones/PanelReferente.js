...
function PanelReferente() {
  ...
  const cargarSolicitudes = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${API_BASE_URL}/api/vacaciones/pendientes-referente`, {
  ...
  const handleResponder = async (aprobado) => {
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/vacaciones/responder-referente/${selectedSolicitud.id}`,
        {
          method: 'PUT',
          credentials: 'include',  // Incluir cookies automáticamente
          headers: {
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
                ...
                <TableRow key={solicitud.id} hover>
                  ...
                  <TableCell>
                    <Typography variant="body2" sx={{ maxWidth: 200 }}>
                      {solicitud.comentarios_empleado || '-'}
                    </Typography>
                  </TableCell>
                  <TableCell align="center">
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth>
        <DialogTitle>
          {selectedSolicitud?.rechazado ? '✗ Rechazar Solicitud' : '✓ Aprobar Solicitud'}
        </DialogTitle>
        ...
        <DialogActions>
          ...
          {selectedSolicitud?.rechazado ? (
            ...
            <Button
              ...
              disabled={respondiendo}
            >
              {respondiendo ? <CircularProgress size={24} /> : 'Aprobar'}
            </Button>
          )}
        </DialogActions>
      </Dialog>
