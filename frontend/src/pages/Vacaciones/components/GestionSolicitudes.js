...
const GestionSolicitudes = () => {
  ...
  const response = await fetch(`${API_BASE_URL}/api/vacaciones/todas-solicitudes`, {
        headers: {
  ...
  const manejarRespuesta = async (estado) => {
    ...
    try {
      setProcesando(true);
      const token = localStorage.getItem('token');
  const response = await fetch(`${API_BASE_URL}/api/vacaciones/responder/${solicitudSeleccionada.id}`, {
        method: 'PUT',
        headers: {
