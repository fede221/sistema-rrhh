...
const LegajoAdmin = () => {
  ...
  const fetchLegajos = () => {
  axios.get(`${API_BASE_URL}/api/legajos`, {
  ...
  const fetchEmpresas = () => {
  axios.get(`${API_BASE_URL}/api/legajos/empresas`, {
      headers: { Authorization: `Bearer ${getToken()}` }
    }).then(res => {
  ...
  ? `${API_BASE_URL}/api/legajos/${formData.id}`
  : `${API_BASE_URL}/api/legajos`;
  ...
  axios.delete(`${API_BASE_URL}/api/legajos/${id}`, {
        headers: { Authorization: `Bearer ${getToken()}` }
      }).then(() => fetchLegajos())
    ...
    try {
      ...
      for (let i = 0; i < datosImportar.length; i += tamanoLote) {
        ...
        try {
          ...
          const response = await axios.post(`${API_BASE_URL}/api/legajos/actualizar-masivo`, 
            payload,
