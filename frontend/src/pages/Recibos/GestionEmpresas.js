...
const GestionEmpresas = () => {
  ...
  ? `${API_BASE_URL}/api/empresas/${selectedEmpresa.id}` 
  ...
  const handleToggleEstado = async (empresa) => {
    try {
  const response = await fetch(`${API_BASE_URL}/api/empresas/${empresa.id}/estado`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${getToken()}`,
          'Content-Type': 'application/json'
  ...
  const handleEliminar = async (empresa) => {
    ...
    try {
  const response = await fetch(`${API_BASE_URL}/api/empresas/${empresa.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${getToken()}`,
          'Content-Type': 'application/json'
