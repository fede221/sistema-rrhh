import { API_BASE_URL, getApiBaseUrl, getServerBaseUrl } from '../../../config';
...
const Recibos = () => {
  ...
  const generarHtmlReciboEmpleado = async (periodo, registros) => {
    ...
    // Obtener la URL base del servidor (sin /api) para construir rutas de uploads
    ...
    if (firmaUrl) {
      ...
      if (firmaUrl.startsWith('http')) {
        fullFirmaUrl = firmaUrl;
      } else if (process.env.NODE_ENV === 'production') {
        fullFirmaUrl = `/uploads/${firmaUrl.replace(/^.*[\\\/]/, '')}`;
      } else {
        fullFirmaUrl = `${serverBaseUrl}/uploads/${firmaUrl.replace(/^.*[\\\/]/, '')}`;
      }
  ...
  const fetchMisLegajos = async () => {
    try {
      const legajos = await apiRequest('/legajos/mis-legajos');
  ...
  useEffect(() => {
    const fetchRecibos = async () => {
      try {
        const data = await apiRequest('/recibos/mis-recibos');
        setRecibos(data);
  ...
  const handleFirmaSubmit = async () => {
    ...
    try {
      ...
      if (legajosUnicos.length > 1) {
        ...
        for (const legajo of legajosUnicos) {
          const res = await fetch(`${API_BASE_URL}/recibos/firmar`, {
            method: 'POST',
            headers: {
  ...
  const generarHtmlRecibo = async (periodo, registros, empresaInfo = null) => {
    ...
    if (logoUrl) {
      ...
      if (logoUrl.startsWith('http')) {
        ...
        fullLogoUrl = `/uploads/${logoUrl.replace(/^.*[\\\/]/, '')}`;
      } else {
        fullLogoUrl = `${serverBaseUrl}/uploads/${logoUrl.replace(/^.*[\\\/]/, '')}`;
      }
...
<body>
  ...
  const handleSubmit = async (e) => {
    ...
    try {
  const response = await fetch(`${API_BASE_URL}/recibos/importar`, {
        method: 'POST',
        body: formData,
  ...
  const handleCancel = async () => {
    ...
    try {
  await fetch(`${API_BASE_URL}/recibos/cancelar`, { 
        method: 'POST', 
        headers: { Authorization: `Bearer ${getToken && getToken()}` } 
      });
      setPolling(false);
  ...
  useEffect(() => {
    ...
    if (polling) {
      interval = setInterval(async () => {
        try {
          const res = await fetch(`${API_BASE_URL}/recibos/progreso`);
          const data = await res.json();
          setProgress(data);
